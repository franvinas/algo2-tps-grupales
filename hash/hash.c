...
bool nodo_hash_guardar(nodo_hash_t* nodo_hash, const char *clave, void *dato) {//mas que crear seria guardar. Porque la estructura ya esta creada

    nodo_hash->clave = malloc((strlen(clave) + 1) * sizeof(char)); // es necesaria esa contante? No, con poner +1 esta bien
...
bool redimensionar_hash(hash_t* hash, size_t nueva_capacidad){
    ...
    for (int i = 0; i < capacidad_actual; i++){
        if (datos_actual[i].estado == OCUPADO){
            hash_guardar(hash, datos_actual[i].clave, datos_actual[i].dato);
...
bool hash_guardar(hash_t *hash, const char *clave, void *dato){
    ...
    if(!nodo_hash_guardar(&hash->datos[posicion], clave, dato)) return false;
...
//Pre: La estructura hash fue inicializada
//Post: En caso de encontrar la clave devuelve el dato , sino NULL
void *hash_obtener(const hash_t *hash, const char *clave){
    size_t  pos = posicion_clave(clave,hash);
    return hash_pertenece(hash, clave) ? hash->datos[pos].dato : NULL;
}
